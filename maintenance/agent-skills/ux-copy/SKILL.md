...
This kit owns its content style: `{KIT}/guidelines/COMPONENTS.md § Content Style` and
`{KIT}/guidelines/EXECUTION.md § 12.8 Content Style Application`, with language and format
rules in `§ 12.8A Localization, Language, And Format Rules`. Two hard ones — the brand name is written as documented
(`{KIT}/guidelines/DESIGN.md § The Brand Name`), and copy never exposes a component or rule
name. Multilingual resilience constrains length: `COMPONENTS.md § Multilingual Content
Resilience`.
