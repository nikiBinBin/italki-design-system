...
`Popconfirm`'s `trigger`, `confirm` and `cancel` are **slots: pass JSX straight in**. An element is portalled
...
strings, so the markup is the component's own: `trigger: ui.button({ … })`.
...
<!-- 待补：Popconfirm 自己那一节（什么时候不该用它）。云端
     components/**/Popconfirm/Popconfirm.prompt.md 里有，见 ../PROMPT-NOTES-LOST.md。 -->
