...
`DropdownMenu`'s `trigger` is a **slot: pass JSX straight in**. An element is portalled
...
<!-- 待补：DropdownMenu 自己那一节（什么时候不该用它）。云端
     components/**/DropdownMenu/DropdownMenu.prompt.md 里有，见 ../PROMPT-NOTES-LOST.md。 -->
