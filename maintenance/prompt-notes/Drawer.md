...
`Drawer`'s `body`, `footer` and `trigger` are **slots: pass JSX straight in**. An element is portalled
...
strings, so the markup is the component's own: `body: ui.button({ … })`.
...
<!-- 待补：Drawer 自己那一节（什么时候不该用它）。云端
     components/**/Drawer/Drawer.prompt.md 里有，见 ../PROMPT-NOTES-LOST.md。 -->
