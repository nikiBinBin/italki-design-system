...
`Popover`'s `actions` and `trigger` are **slots: pass JSX straight in**. An element is portalled
...
<!-- 待补：Popover 自己那一节（什么时候不该用它）。云端
     components/**/Popover/Popover.prompt.md 里有，见 ../PROMPT-NOTES-LOST.md。 -->
