...
assert(catalog.includes('buttonComponent({ label: "Reset", variant: "secondary", size: 40, shape: "default", demo: "filter-reset" })') && catalog.includes('buttonComponent({ label: "Show teachers", variant: "red", size: 40, shape: "default" })'), "Filter modal footer must use the standard secondary and primary 40px dialog actions");
assert(catalogStyle.includes(".filter-pattern-detail { width: 100%; margin: 0; display: grid; justify-items: center; }") && catalogStyle.includes(".filter-pattern-detail > .ui-modal-stage--inline { width: 100%; justify-content: center; }"), "Filter pattern trigger must be centered without changing discovery filter placement");
...
assert(catalog.includes('label: "Reset", variant: "secondary", size: 40, shape: "default", demo: "filter-reset"') && catalog.includes('else if (type === "filter-reset") {'), "Filter Reset must dispatch the shared resetFilters behavior");
