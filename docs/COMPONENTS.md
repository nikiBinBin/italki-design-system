...
- **Backup** — `Assets/Icons/*.svg`, for cases the main set does not cover. Always try the main set first.
...
**`top-nav`** — Headless horizontal navigation surface. Call `topNav({ id, leading, center, trailing, ariaLabel, sticky, demo })` with supplied slots; it owns only a 72px white bar, one bottom `Foreground/Border` separator, 24px desktop edge inset, and the reading order `leading` → `center` → `trailing`. It is sticky to the top of the page by default; pass `sticky: false` for the rare surface that should scroll away with the content. It stacks below the modal layer, so an open dialog always covers it. It does not supply global text-navigation items, account data, routes, language choices, search results, filters, or booking behavior.
