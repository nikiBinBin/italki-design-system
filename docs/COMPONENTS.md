...
`package-card` is the specialized lesson-package comparison form: `discount` is the optional offer label, `price` and `period` state the per-lesson cost, `description` gives the supplied package context, and its anchored footer contains `quantity`, `quantityLabel` (default `Lessons`), optional `originalPrice`, and `totalPrice`. It reuses the approved `Assets/Icons/16px/category-sm.svg` asset as a masked token-colored glyph; no new offer icon is introduced. Use `layout: package-grid` for a comparable set: four columns on wide surfaces, two at mid-width, then one on narrow widths. Its deliberate 280px minimum height provides a consistent comparison baseline and pins all quantities and totals to the bottom edge; it is not a generic content-card height. A selected package card always displays the shared right-corner marker alongside its strengthened boundary, while retaining radio-group accessibility without adding a visible second control. Use it only for a supplied, prepaid lesson-package choice—not a plan, promotion, bundle, checkout summary, or arbitrary pricing card.
