...
- `catalog-runtime/component-api.json` is a generated searchable index of every accepted prop, runtime default, state, enum, and allowed Foundation token family. Generate these indexes with `npm --prefix maintenance run build:api`; do not edit them manually.
...
- **Foreground/Placeholder-text** (#9C9CAC, alias `Gray/400`): Input placeholder and empty-value text, and the quiet global text navigation in the logged-out `top-nav`, which darkens to `Foreground/Primary-text` on hover.
...
- **Backup** — `Assets/Icons/ui/*.svg`, a vendored general-purpose set on the same 24px canvas, for cases the main set does not cover.

**Search the italki set first; reach for the backup only when nothing there fits.**
The first two sets are the ones italki drew for this product: they carry its
line weight, its corner treatment, and its vocabulary, so a screen built from
them reads as one hand. The backup is a general library — correct and approved,
but drawn by someone else — and every glyph taken from it is a small break in
that voice. So the order is not a preference, it is the rule:

1. Look in the **Main** set (24px) — or **Small** if the context is dense.
   The Catalog's **Icon library** board is this set, searchable by name.
2. Only if no icon there carries the meaning, take one from the **Backup**.
   The Catalog's **General icon set** board is that set, on its own so it can
   never be mistaken for italki's own.
3. Never invent a name or pull from a Figma URL. If neither set fits, say so
   and request the icon rather than approximating it with a near-miss.

A near-miss from the main set beats an exact match from the backup only when the
near-miss still says the right thing; when it does not, the backup is the correct
choice and needs no apology. What is never correct is reaching for the backup
first because it is larger and easier to search.

**Five glyphs sit in both sets, and the Main copy is the real one.**
`plus-circle`, `minus-circle`, `chevron-right`, `home-01` and `youtube` are the
same drawing at `Assets/Icons/` and at `Assets/Icons/ui/` — they were adopted
into the main set because component contracts name them: Number stepper's two
actions, List's marker, Sidebar's section arrow, Breadcrumb's home, Footer's
YouTube. Reference the Main path. Finding one of them in the backup folder is
not the backup set leaking into the library; the Catalog lists each once, on the
Icon library board, and the runtime resolves a bare name to the Main copy.
...
Every functional pictogram in `index.html` must render an approved asset from the Icon Library: `Assets/Icons/` first, then `Assets/Icons/16px/`, then `Assets/Icons/` only when the first two sets have no suitable asset. Do not use Unicode symbols, CSS-drawn pictograms, inline SVG, external icon URLs, or a visually similar substitute for a real UI icon. Text punctuation, numeric values, status geometry, and product imagery are not UI icons. Before introducing an icon, check the Icon Library and its documented size. If the required concept or required 16px/24px variant is missing, stop and report the gap with the intended action and suggested asset name; do not create an ad hoc icon or scale an asset to imitate an unavailable size.

### Image Library

`Assets/Images/` is the third approved asset root, alongside `Assets/Icons/` and `Assets/Flags/`, and it holds content imagery rather than component primitives. It is one pool, not a set per component: the same photograph is an Avatar at 40px, a Card's media at 16:9 and a Video's poster, so the file is not filed by the component that happens to use it. Every source is a 16:9 crop; `faces/` holds the square head-and-shoulders cut of each one, which is what an Avatar should be given — a half-body portrait in a 40px circle is a shirt with a head on it. The Catalog's **Image** foundation card lists every file with its exact path; it is generated from `catalog-runtime/image-manifest.js` by `maintenance/scripts/build-image-manifest.mjs`, and `validate-contracts.mjs` fails when the two disagree, so a file dropped into the folder cannot silently miss the card.

- **Pass the whole path.** `avatar({ image: "Assets/Images/faces/04.jpg" })`, `card({ media: "Assets/Images/covers/04.jpg" })`. Unlike an icon, an image has no bare-name lookup: `resolveIcon` searches the icon manifest only, and an image name reaching it throws.
- **These are reference photography.** They exist so a screen can be composed and reviewed with something real in it. Replace them with the product's own imagery before anything ships; a delivered page still carrying `covers/04.jpg` has not been finished.
- **An avatar is not content imagery.** Avatars, flags, and icons are component primitives with their own contracts. The ratio rules on the Image card — 16:9 and 3:1 — govern content imagery only.
...
| `Radius/MD`   |    8px | **Default workflow buttons**, inputs, and uncontracted surfaces 48px or shorter                                                            |
| `Radius/LG`   |   12px | **Cards and Dialogs** — teacher, booking, filter, review, 520px and 744px Dialogs; uncontracted surfaces over 48px through 120px         |
| `Radius/XL`   |   16px | Large drawers, sheets, and uncontracted surfaces taller than 120px                                                                          |
| `Radius/Full` | 9999px | Explicitly pill-shaped components and named pill variants                                                                                    |
...
| ---------------- | -------------------: | ------------------------------------------------------------------------------------------------ |
| 48px and below    |      `Radius/MD` 8px | Compact uncontracted controls and surfaces                                                         |
| Over 48–120px     |     `Radius/LG` 12px | Standard controls, cards, booking rows, and actionable blocks                                    |
| Over 120px        |     `Radius/XL` 16px | Large uncontracted panels and containers                                                           |
...
- Apply the 48px-and-below `Radius/MD` rule only when the component has no explicit radius contract. Do not override a documented component radius with this foundation default.
...
- For components without an explicit contract, use `Radius/MD` through 48px, `Radius/LG` through 120px, and `Radius/XL` above 120px.
- `Radius/Full` is reserved for an explicitly pill-shaped named component or variant. Do not use it merely because the component is compact.
- Do not increase an uncontracted component's radius merely to imitate a softer corner.
...
| Foreground/Divider | #F5F6F9 | #3A3A4D |
...
**`section-intro`** — Reusable section heading for a bounded page region. Call `sectionIntro({ id, eyebrow, title, description, action, size, alignment, headingLevel, ariaLabel })`. It receives a required visible `title` (or an explicit accessible name for exceptional visually hidden contexts), optional supplied eyebrow and description, one supplied local `action` slot, `size` (`default | compact`), `alignment` (`start | center`), and semantic `headingLevel` (`2 | 3`, default `2`). Default uses 24px / 32px title type; compact uses 18px / 24px. Eyebrow is 12px / 600 / 18px and description is 14px / 20px with secondary text; title uses title text. The start-aligned form puts one supplied action at the trailing edge on wide layouts and moves it below the copy on narrow layouts. Centered form stacks its action after the copy. Use it to introduce a self-contained section where a title, concise context, or one local continuation must stay together. It does not create page hierarchy, determine section spacing, fetch content, infer an action, own navigation, or replace `panel` headers, `modal` titles, Card titles, or a product-specific hero. The parent supplies the heading level, copy, action behavior, surrounding layout, and any responsive page composition. Do not use it merely to label a divider or repeat a page title.
...
**`tag`** — Headless compact classification marker or supplied selected-value token. It receives supplied visible content, optional approved `leadingIcon`, one semantic tone (`neutral`, `info`, `success`, `warning`, or `error`), visual `variant` (`default | status | promo`), `size` (`24 | 32 | 40`, default `24`), `removable`, accessible text, overflow behavior, and removal callback through props. The default form presents one static attribute, duration, level, status, or concise comparison fact; it never changes selection, opens a menu, filters a collection, changes a route, or receives button, checkbox, or radio semantics. `status` keeps its text neutral and applies the supplied semantic color to the surface; it can pair a 16px status icon with records such as an active package, scheduled time, completed lesson, action-required notice, or waiting notice. `promo` uses the existing Plus gradient token for brief supplied promotional or completion copy. The neutral default inside a white surface uses `Foreground/Divider` (#F5F6F9) fill with `Foreground/Primary-text` (#515164) text and no border; on a gray page surface it uses white fill with a 1px `Foreground/Border` outline. All named Tag sizes use `Radius/Full`: 24px uses 12/18 type with 8px horizontal inset; 32px uses 14/20 type with 12px horizontal inset; 40px uses 14/20 type with 16px horizontal inset. Status and Promo Tags at 32px retain 12/18 type with 12px left and 16px right inset to match their icon-led reading order. A documented size switch updates every Tag in its supplied container through the shared `setTagPresentation(container, { size })` implementation; Catalog must not imitate the size change with local CSS. A removable Tag exposes one separately labelled removal action sized 16px, 20px, or 24px to match the supplied Tag size and asks the parent to remove the supplied token; use it for selected values inside a control such as `select` multiple, not for a filter choice. Its approved `Assets/Icons/16px/cross-sm.svg` mark rests at muted secondary emphasis and returns to full emphasis only on hover or keyboard focus. A Tag never toggles itself: a selected, hoverable, or keyboard-selectable choice is a `chip`, not a Tag.
...
**`progress`** — Controlled continuous-progress primitive for a supplied bounded amount, such as lesson package use, learning completion, or a measurable processing task. Call `progress({ value, max, status, type, size, showLabel, indeterminate, ariaLabel })`. `type` is `line` (default), `circle`, or `semicircle`; circular forms accept the registered 80px or 120px `size`. The line has an 8px pill track and an optional readable percentage at its trailing edge. Circle and semicircle place the percentage inside the supplied progress shape; they use the same 12px stroke and semantic colors. `status` is `default`, `success`, or `error`. `indeterminate` is reserved for a line whose known structure is loading; it cannot be used for circular shapes, which always communicate a supplied determinate amount. It renders one programmatic `progressbar`, but never calculates the ratio beyond the supplied `value / max`, estimates time remaining, fetches work state, or represents named multi-step position; use `stepper` and its `flow-progress` variant for stages, and `skeleton` for unknown loading structure.

### Data Display

**`card`** — Headless discrete-object surface for supplied media, header, body, metadata, and footer slots. It receives `interactive`, `outlined`, optional media source and alt text or `mediaPlaceholder`, `mediaRatio` (`16:9 | 3:1`), supplied eyebrow, title, body, metadata and footer slots, `density` (`compact | default | comfortable`), accessible name, optional supplied `href`, and demo identifier through props. `mediaPlaceholder` is the neutral media fallback: `Background/Hover` with the white italki logomark, matching List placeholders. Static Card renders an `article`, `Background/Card`, `Radius/LG` (12px), and `Shadow/Card`; its optional outlined state uses `Shadow/Stroke card` rather than a CSS border. An interactive Card renders one root link or button, has no resting border, and uses `Shadow/Card-Hover` for hover and focus. The interactive root must not contain a second interactive descendant. The supplied media is clipped to the Card boundary. Default body inset is 16px, compact is 12px, and comfortable is 24px. Use Card for a discrete content object or one actionable destination; use `panel` for a related region with a stable internal header, and use a Pattern for teacher, booking, payment, or other product-specific compositions. Do not nest Cards, use Card as page-section chrome, fetch its object data, infer actions, or decide navigation.

**`list`** — Ordered local records with consistently aligned supplied content. Call `list({ id, items, size, variant, divided, ariaLabel, demo })`. It receives a non-empty ordered `items` collection; each item supplies a stable `id`, readable `label`, optional `description`, optional `content`, optional `avatar` slot, optional supplied `likes` and `comments` counts, optional trailing content, an approved `image` plus `imageAlt` or `imagePlaceholder`, optional `href`, disabled meaning, accessible name, and demo event. `size` is `small` (48px row with 12px horizontal inset), `default` (56px row with 16px inset), or `large` (72px row with 24px inset). Hover and focus fill the complete row, including its named size inset, without a rounded row surface. `variant` is `default`, `avatar`, `image`, or `content`. Avatar List requires one supplied Avatar slot per item and uses the established Avatar component; Right Image List requires one approved local image or `imagePlaceholder` per item and renders it at the trailing edge in a 96 × 54px (16:9) `Radius/MD` frame. The placeholder is a non-semantic, light-gray field with the italki logomark. Content List requires an Avatar, supplied body content, and one approved trailing image or placeholder; its body and interaction counts align to the Avatar's left edge, and its supplied likes and comment counts use the approved `upvote-sm` and `comments-sm` icons. Its supporting gaps are 4px between text levels, 12px between columns, its trailing image is 160 × 90px (16:9), and it has 24px horizontal and 12px vertical inset. It does not infer author identity, format a timestamp, calculate engagement, fetch media, or mutate a reaction. `divided` is true by default and adds one `Foreground/Divider` rule only between adjacent rows. An item becomes an accessible link when it supplies `href`; a supplied demo event renders a button for controlled local interaction. Use List for concise, same-level records such as lesson resources, schedules, settings destinations, or recent activity. It does not fetch records, determine order, calculate unread state, choose an icon, create a product-specific card, paginate, filter, or decide the row destination. Use Table for column-by-column comparison, Card for a discrete content object, and Timeline for chronological event history.
...
**`tabs`** — Controlled local content-view composite for switching between closely related panels in one page region or invoked surface. It receives a stable `id`, ordered `items` (`id`, visible label, supplied panel slot, optional approved 16px icon, optional compact count, and disabled state), active id, `orientation` (`horizontal`), `variant` (`default | red-line`), activation mode (`manual | automatic`), optional trailing-action slot, accessible label, and demo identifier through props. The default is a horizontal line tab bar: transparent at rest; hover and keyboard focus use a `Radius/MD` `Foreground/Divider` surface; the active label uses title color and its 2px `Gray/600` indicator scales in over 160ms. `red-line` follows the DS3.0 selected treatment: 48px tab height, 16/24 bold label, and a centered 32×4px `Primary/Main` red ink bar with `Radius/XS`; it preserves the same role, keyboard, panel, and overflow behavior as the default. An activated panel enters once with a restrained 180ms fade and 4px rise; both animations are disabled for reduced-motion preferences. A disabled tab cannot be activated and uses disabled text. Arrow keys, Home, and End move focus; automatic activation updates the supplied local panel while manual activation waits for an explicit trigger. The parent supplies item order, content panels, lifecycle, overflow policy, and active state; an overflowing horizontal tab list scrolls rather than wrapping into an ambiguous second row. It keeps one local content context visible; it never changes routes, represents task completion, replaces page navigation, owns a global navigation list, or hides unrelated sections. Card-style editable workspaces, closeable browser-like views, and route-level navigation are separate product compositions, not core Tabs.

**`segmented-control`** — Compact controlled single-select mode or view switch for two to five immediately comparable options, such as list/grid or day/week. It receives ordered options, selected id, `size` (`32 | 40 | 48`), `shape` (`pill | rounded`), `contentType` (`text | icon | role`), disabled state, accessible label, and callbacks through props. `pill` is the default; `rounded` uses `Radius/MD` (8px). `icon` uses a single approved 16px icon per option and requires a visible-label equivalent through each option’s accessible label; use it only for instantly recognizable local modes, never alongside text in the same segment. `role` is a two-option account-role switch: it pairs the supplied 24px role icon with its label in a larger segment, keeps the selected role on a card surface, and shows the shared switch affordance between the options. Shape is a local visual treatment only: keep one shape consistent within one decision set. It does not own a tab panel, route, filter data, form submission, or a multi-step decision; use `tabs`, `choice-group`, or `stepper` when those semantics apply.
...
**`top-nav`** — Headless horizontal navigation surface. Call `topNav({ id, leading, center, trailing, ariaLabel, sticky, demo })` with supplied slots; it owns only a 72px white bar, one bottom `Foreground/Divider` separator, 24px desktop edge inset, and the reading order `leading` → `center` → `trailing`. It is sticky to the top of the page by default; pass `sticky: false` for the rare surface that should scroll away with the content. It stacks below the modal layer, so an open dialog always covers it. It does not supply global text-navigation items, account data, routes, language choices, search results, filters, or booking behavior.

**`top-nav` visitor variant** — `variant="logged-out"` is the signed-out bar. It is not a trimmed signed-in bar: the centre column is removed rather than emptied, so there is no search, and the mark sits at the leading edge with the whole row collected at the trailing edge. It takes `links` (ordered `{ id, label, icon, href }`), `localeLabel` and `localeOptions` for one combined language-and-currency control, `actionLabel` for the visitor action, `mobileActionLabel`, `menuOpen`, `menuActionLabel`, and `menuLocale` / `menuCurrency` (`{ label, options, selected }`). Below 744px of its own width it becomes the phone bar — menu trigger, mark, one 32px action — and the destinations move into a left Drawer that this variant composes from the shared `drawer` contract with `stage="inline"` and `titleHidden`. Its links use `Foreground/Placeholder-text` at `14px / 500 / 22px` and darken to `Foreground/Primary-text` on hover or keyboard focus; its `icon` is required only for the menu rows, which are `16px / 500 / 24px` on `Foreground/Title`. It supplies no destinations, no locale or currency values, no session state, and no sign-up outcome.

**`top-nav-context`** — Subcomponent for a supplied contextual menu. Call `topNavContext({ id, mode, selected, options, open, ariaLabel, demo })`, where `mode` is `labelled | compact | plain`, `selected` supplies `{ id, label, flag }`, and each option supplies the same stable id, label, and approved local flag asset. `plain` is the flagless mode for a context that no single country stands behind — a combined language and currency choice — and it refuses a flag rather than accepting an optional one; the other two modes require it. It owns the menu disclosure relationship and local selection presentation, but does not derive or persist a context choice. `labelled` uses a 24px visual, `14px / 700 / 20px` label, and the approved 24px `arrow-down-sm` asset with 8px gaps and 8px trigger inset; `compact` omits only the visible label. Resting state has no fill or outline; hover and open use `Foreground/Divider`, while direct hover of an open trigger may use `Background/Hover`. Its supplied 208px menu uses `Radius/XL`, 8px container padding, 4px row gaps, and 40px options with a 24px visual, 12px visual-to-label gap, and `14px / 600 / 20px` labels.
...
Top-nav actions are supplied slots and must use the existing `button` renderer. A signed-in action uses `button({ variant: 'emphasis', size: 40, shape: 'pill' })`, optionally with one approved 24px leading icon, so padding, text, focus, disabled, and loading behavior remain the Button contract. The logged-out action uses `secondary` at the same size and shape, and 32px on the phone bar: a visitor who has not chosen anything yet is being offered a way in, not pushed through one, and the page's own red CTA stays the single conversion action. `top-nav` never creates a parallel action style. On narrower widths, the parent supplies slot compression or the alternate presentation defined by `EXECUTION.md`. The reusable UI component remains distinct from the authenticated `workspace-header` pattern in `PATTERNS.md`.

**`sidebar`** — Controlled vertical navigation surface. It receives a stable `id`, `variant` (`normal | plus`), `collapsed`, ordered primary `items`, optional disclosure `sections`, optional `moreItems`, `moreOpen`, supplied footer slot, accessible label, `sticky`, and demo identifier through props. The rail is as tall as the viewport and sticky to the top: the page scrolls, the rail does not. Its own overflow is internal — a long navigation scrolls within the rail while the footer stays on the floor — so the consumer supplies no height and no scroll container. Pass `sticky: false` for a rail that should scroll away with the page. Like the top nav, it stacks below the modal layer. One host constraint the component cannot work around: `position: sticky` is inert inside an ancestor with `overflow: hidden`, so a shell placed in a clipped container scrolls away regardless of this prop. Audited across six host layouts and four viewports; that is the only case where the shell does not pin. A primary item supplies id, label, approved Icon-library asset, active and disabled state; one item may be the More trigger. Use `fixed: true` for destinations that must stay at the top: they never expose Pin controls or dragging. All other primary destinations are pinnable: their right-side solid Pin action removes them from the primary list and returns them to More; More rows expose an outline Pin action that restores them before More. Pinnable rows can be dragged to reorder within their current list, while fixed rows always retain their relative positions. A section supplies id, label, `open`, and ordered child items. A child item may supply an approved leading slot, numeric prefix, optional 1px divider, and secondary text; these values are supplied content, not user, teacher, lesson, chat, or route data inferred by the component. `moreItems` is the overflow roster in its canonical order, not a snapshot of what the menu currently holds: a destination pinned out to the primary list is declared in both, rendered only in `items`, and returns to its own place in the menu when it is unpinned — with the roster's own label and icon, which may differ from the nav's. More items supply id, label, approved icon, disabled state, and `dividerBefore` on the first member of each group. A divider is drawn before the first *visible* member of a group, so pinning a group's leader out does not merge the rest of it into the group above. The root is a named `aside` with a named destination `nav`; More is an anchored `menu`, and sections expose `aria-expanded` / `aria-controls`.
...
**`drawer`** — Controlled edge-anchored focused-task surface. It receives an id, supplied trigger or fallback trigger label, title, body and footer slots, `open`, placement (`left | right | bottom`), size (`default | wide`), `stage` (`demo | inline`), `titleHidden`, close affordance, mask and keyboard dismissal permissions, focus-return target, and close callbacks through props. A panel whose task is about a person also takes `titleLeading` and `subtitle`: `titleLeading` is a slot before the heading for one identity element — an Avatar, not a stack of controls — and `subtitle` is a one-line slot under it for the supplied summary that names who or what the task concerns. Both are optional; with neither, the header emits the heading alone and is unchanged. They do not carry actions, a second heading, or wrapping text: the subtitle is one line and truncates. `stage` reads exactly as Modal's: `demo` is the bounded preview surface for component documentation, and `inline` leaves only the trigger in normal layout and hangs the panel off the viewport. `titleHidden` keeps the dialog's accessible name and takes the visible heading and its header rule out of the picture, for a panel whose contents name themselves; it never removes the name. It uses `Special/Mask` and one `Background/Card` surface. A right Drawer is flush and square-cornered; left and bottom Drawers use `Radius/XL` (16px) only on their exposed corners. Footer actions align to the end with a 16px gap. On open and close, the panel slides 220ms from or toward its placement edge while the mask fades; reduced-motion preference shows the final state without the movement. It is appropriate for a related secondary task that needs more vertical space than a popover while leaving the originating page context visible. It traps focus while open, closes on Escape only when permitted, and returns focus to its invoking control after dismissal. It does not own a page route, fetch options, validate a business task, persist edits, or decide whether an apply action succeeds.
...
**`selection`** — Controlled, information-rich selectable card. It is one semantic shell with independent `contentType` (`standard | icon-simple | icon-card | package-card | lesson-options | avatar | payment-icon`), `selectionMode` (`radio | checkbox`), visual `state` (`default | hover | focus | disabled`), selected state, optional `selectedMarker`, and supplied content slots (`label`, optional `subtext`, `description`, `leading`, `badge`, `price`, `period`, `discount`, `quantity`, `quantityLabel`, `originalPrice`, and `totalPrice`). Content is consistently left aligned. `standard` places the shared Radio or Checkbox visual primitive before text; standard information cards place the same primitive at the far right. The Selection root remains the only interactive element, so it never nests an interactive Radio or Checkbox. `icon-card` adds a bounded header and body but remains the same Selection component, not a separate product card. The parent supplies all labels, values, imagery, pricing, and selected values; Selection owns only its card surface, local focus, accessible radio or checkbox semantics, and local selected presentation.

Selection uses a white surface, 1px `Foreground/Border` boundary, `Radius/LG`, 16px inset, compact 14px text, and one 16px control scale. `icon-card` applies that 16px inset to both its header and body and renders its supplied leading icon directly, without an extra icon tile. A standalone card and a `layout: stack` group cap reading width at 560px; `layout: grid` is the named two-column comparison form and contracts to one column on narrow widths. Selected keeps the same outer dimensions and strengthens the existing boundary with inset `Gray/600`. `selectedMarker` is an optional selected-only 20px `Gray/600` corner marker; it is clipped inside the Selection radius, never positioned by page-level CSS. It is the card mode's sole visual selected indicator, so a card configured with `selectedMarker` hides its regular Radio or Checkbox glyph while retaining its radio or checkbox semantics. Height follows supplied content rather than a fabricated fixed height. Hover uses `Foreground/Divider`; focus strengthens the inset title boundary without an outer halo; disabled uses a quiet divider surface and no interaction. Radio cards are mutually exclusive: a group exposes one named `radiogroup`, one selected card, roving tab focus, and arrow-key movement. Checkbox cards are independent multi-select choices and each exposes checkbox semantics. On narrow widths, `icon-card` may stack its badge above price and description while preserving the same semantic component. Use Selection when each choice needs evidence or comparison content; use bare Radio or Checkbox for short choices, Chip or Segmented control for compact modes, and Button only for an action.

`package-card` is the specialized lesson-package comparison form: `discount` is the optional offer label, `price` and `period` state the per-lesson cost, `description` gives the supplied package context, and its anchored footer contains `quantity`, `quantityLabel` (default `Lessons`), optional `originalPrice`, and `totalPrice`. It reuses the approved `Assets/Icons/16px/category-sm.svg` asset as a masked token-colored glyph; no new offer icon is introduced. Use `layout: package-grid` for a comparable set: its 200px minimum card width lets the grid fill each available row with as many cards as fit, then wraps automatically as the surface contracts. It uses a compact 224px comparison baseline, pins quantities and totals to the bottom edge, and truncates longer package context after three lines to preserve that baseline; it is not a generic content-card height. A selected package card always displays the shared right-corner marker alongside its strengthened boundary, while retaining radio-group accessibility without adding a visible second control. Use it only for a supplied, prepaid lesson-package choice—not a plan, promotion, bundle, checkout summary, or arbitrary pricing card.

`lesson-options` is the course-led selection form. Use `selectionGroup({ contentType: "lesson-options", selectionMode: "radio", courseTitle, courseMeta, options })` for the detailed card: its supplied `options` are duration rows with visible `label` and `price`, and the group owns the only `radiogroup` semantics, roving tab stop, row-level radio state, and selected course corner marker. Supply `courses` with `selected` and `selectedDuration` to render the expandable course list: an unselected course summary previews its duration rows while hovered or focused, then collapses them when that temporary interaction leaves; click selects and persistently expands it. Activating the current course summary again clears it and collapses its rows. Use standalone `selection({ contentType: "lesson-options", label, description, price })` for the compact course-summary form, where the whole card is one course-level choice. These are intentionally separate interaction levels: duration rows select a time within a known course, while the compact card selects the course itself. Do not combine the two into nested interactive controls. Both forms use one neutral card surface and the shared selection boundary; they do not calculate pricing, lesson eligibility, or content level.
