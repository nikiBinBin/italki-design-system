...
1. Look up the component contract in `COMPONENTS.md` and its accepted props, defaults, states, tokens, assets, and **Production Mapping** in generated `docs/reference/COMPONENT_API.md` or `catalog-runtime/component-api.json`. The registry remains `catalog-runtime/contracts.json`; the API index is its searchable implementation view.
   - The Production Mapping decides the code path: `aligned` means use the listed Panda export or approved wrapper; `adapter` means compose the stated Panda primitive with the existing product behavior; `composed` means reuse the named business assembly rather than treating it as a Panda primitive. A `gap` or `legacy` mapping must be reported, not worked around with a local replacement.
   - When a component has a Panda API mapping, resolve every Catalog prop in generated `docs/reference/PANDA_API.md` or `catalog-runtime/panda-api.json` before writing code. `direct` maps to the stated Panda prop; `adapter` requires the named product composition; `catalog-only` is never a production prop; `gap` must be reported. Never copy Catalog prop names into Panda by assumption.
   - Before applying a Catalog color in production, look it up in generated `docs/reference/FOUNDATION_API.md` or `catalog-runtime/foundation-api.json`. `aligned` is safe to use through the listed Panda token; `override` needs a reviewed ConfigProvider theme decision; `gap` must be reported and cannot be approximated with another Panda color.
