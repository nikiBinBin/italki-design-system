...
- `COMPONENTS.md` defines Foundations, Content Style, and reusable UI component contracts.
- `PATTERNS.md` defines italki product compositions and object relationships using those components.
- `EXECUTION.md` defines page-level responsive behavior, interaction, state, accessibility behavior, and implementation constraints.
- `COMPONENT_CATALOG.html` is the visual browser for reviewed design-principle content, Foundations, components, and documented product patterns. It keeps components and patterns in separate navigation groups and is not an additional prose specification.
